...
// In-memory fallback cache (per instance, lost on cold start)
const _mem = { retail: { token: null, expiry: 0 }, fresh: { token: null, expiry: 0 } };

async function getToken(tokenUrl, clientId, clientSecret, cacheKey) {
  // 1. Check in-memory cache (fastest — same instance)
  const m = _mem[cacheKey];
  if (m.token && Date.now() < m.expiry) return m.token;

  // 2. Check Redis cache (shared across all serverless instances)
  const redisKey = `wcl:token:${cacheKey}`;
  const cached   = await redisGet(redisKey);
  if (cached) {
    // Warm the in-memory cache for subsequent calls on this instance
    m.token  = cached;
    m.expiry = Date.now() + 300_000; // treat as good for 5 more min in-memory
    return cached;
  }

  // 3. Fetch a fresh token from WCL
  ...
  if (res.status === 429) throw new Error('WCL rate limit hit while fetching auth token');
  const data = await res.json();
  if (!data.access_token) throw new Error(`WCL auth failed (${cacheKey}): ${JSON.stringify(data)}`);

  const ttl = (data.expires_in ?? 3600) - 120; // 2-min buffer before expiry
  // Store in Redis so other instances reuse it
  await redisSet(redisKey, data.access_token, ttl);
  // Warm in-memory cache
  m.token  = data.access_token;
  m.expiry = Date.now() + Math.min(ttl, 300) * 1000;
...
async function query(apiUrl, token, gql, variables = {}) {
  ...
  if (res.status === 429) throw new Error('WCL rate limit hit — too many requests. Please wait a moment and try again.');
