...
/** Query the Fresh TBC WCL API (fresh.warcraftlogs.com) — requires WCL_FRESH_CLIENT_ID/SECRET env vars */
export async function wclFreshQuery(gql, variables = {}) {
  const id     = process.env.WCL_FRESH_CLIENT_ID;
  const secret = process.env.WCL_FRESH_CLIENT_SECRET;
  if (!id || !secret) throw new Error('WCL_FRESH_CLIENT_ID and WCL_FRESH_CLIENT_SECRET are not set. Register a client at https://fresh.warcraftlogs.com/api/clients');
