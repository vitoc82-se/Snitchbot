...
export function relevantPotKeys(cls, role) {
  ...
  if (role === 'tank') {
    // Ironshield (survival) potion counts for any tank — a defensive pop
    // satisfies the potion requirement just like Haste.
    if (cls === 'Paladin') return ['destruction_potion', 'haste_potion', 'mana_potion', 'survival_potion'];
    return ['haste_potion', 'survival_potion'];
  }
...
// Total potions consumed across all categories — used to rank star players.
export function totalPotions(p) {
  return (Number(p.haste_potion) || 0)
       + (Number(p.destruction_potion) || 0)
       + (Number(p.mana_potion) || 0)
       + (Number(p.survival_potion) || 0);
}

// Star player: brought and used *everything* — full flask/elixir coverage, food,
// scrolls, a relevant in-combat potion, and the right weapon buff. Used by /live
// to spotlight the best-prepared raiders (max 3 per kill, ranked by totalPotions).
export function isStar(p) {
  const flaskOk = (p.flask || p.battle_elixir) && (p.flask || p.guardian_elixir);
  if (!flaskOk) return false;
  if (!p.food) return false;
  if (!p.scrolls) return false;
  const rel = relevantPotKeys(p.class, p.role);
  if (rel.length > 0 && !rel.some(k => p[k])) return false;  // must have used a relevant pot
  const type = weaponBuffType(p);
  if (type === 'oil'   && !p.weapon_oil)                     return false;
  if (type === 'stone' && !(p.weapon_stone || p.windfury))   return false;
  return true;
}
